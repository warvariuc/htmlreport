...
this.clipboard_range = new Array();
this.clipboard_mode = "";
this.active = "";
...
function display() {
    ...
    if (sys.isWriteable) {
        ...
        if (sys.saveMethod)
            out += "<a href='#' onclick='sys.saveMethod(); return false;' accesskey='s'>" + trans("Save") + "</a> - ";
    }
...
function sort(asc) {
    if (sys.multiRange.length > 0) {
        ...
        if (asc)
            ...
            cCells = cCells.sort(sortCells).reverse();
        for (var row = cRange[0]; row <= cRange[2]; row++) {
            for (var col = cRange[1]; col <= cRange[3]; col++) {
                if (!sys.cells[row])
                    sys.cells[row] = new Array();
        ...
        if (asc)
            ...
            sys.cells = sys.cells.sort(sortCells).reverse();
    }
    display();
}
