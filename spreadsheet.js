$(document).ready(function() {
	$(document).on("error", function(msg, url, l) {
	...
	$(document).on("click", "#table", function(e) {
		...
		if (!td.length)
			return
		var indexes = td.attr('id').split('_');
		mouseoverCell(parseInt(indexes[0]), colNo = parseInt(indexes[1]));
	});

	$(document).on("mousedown", "#table", function(e) {
		if (e.which != 1)
			return;
		// need left button
		var td = $(e.target).closest("td");
		if (!td.length)
			return
		var indexes = td.attr('id').split('_');
		var rowNo = parseInt(indexes[0]), colNo = parseInt(indexes[1]);
		mousedown(rowNo, colNo);
	});

	$(document).on("mouseup", "#table", function(e) {
		var td = $(e.target).closest("td");
		if (!td.length)
			return
		var indexes = td.attr('id').split('_');
	...
	$(document).on("mouseover", "#table", function(e) {
		...
		var indexes = td.attr('id').split('_');
		var rowNo = parseInt(indexes[0]), colNo = parseInt(indexes[1]);
		buildStatus(rowNo, colNo);
	});

	$(document).on("dblclick", "#table", function(e) {
		var td = $(e.target).closest("td");
		if (!td.length)
			return
		var indexes = td.attr('id').split('_');
		var rowNo = parseInt(indexes[0]), colNo = parseInt(indexes[1]);
		editCell(rowNo, colNo, 0);
	});
...
function keypress(event) {
	if (!getObj("value") || sys.active == "code")
		return true;
	...
	if (sys.active == "dimensions") {
		if (keyCode == 13) {
			sys.rows = parseInt(getObj("rows").value);
			sys.cols = parseInt(getObj("cols").value);
			sys.active = "content";
			display();
			ret = false;
		}
	...
	if (getObj("styling").disabled) {
		...
		if ((ctrl && (keyCode == 88 || keyCode == 120)) || (shift && keyCode == 46)) {// ctrl-x = cut || shift-del = cut
			...
			cutcopy("copy", "#DDDDFF");
			ret = false;
		} else if (((ctrl && (keyCode == 86 || keyCode == 118)) || (shift && keyCode == 45)) && sys.clipboard_mode != "") {// ctrl-v = paste || shift-ins = paste
			paste();
			ret = false;
		}
		...
		if (!alt && !ctrl && keyCode != 0) {
			if (keyCode == 33) {// page up
				...
				goLeft();
				ret = false;
			} else if (!shift && keyCode == 46 && sys.isWriteable && confirm("Really empty cell(s) ?")) {
				removeSelectedCell();
				ret = false;
			} else if ((keyCode < 33 || keyCode > 40) && keyCode != 46 && keyCode != 45 && keyCode != 16 && keyCode != 17 && keyCode != 18) {
...
function display() {
	...
	out += "&nbsp;";
	if (sys.isWriteable) {
		out += "<a href='#' onclick='if (confirm(\"Really close without saving changes ?\")) load(sys.initData); return false;' accesskey='n'>New</a> - ";
		out += "<a href='#' onclick='loadCode(); return false;' accesskey='l'>Load</a> - ";
		if (sys.saveMethod)
			out += "<a href='#' onclick='sys.saveMethod(); return false;' accesskey='s'>Save</a> - ";
	}
	out += "<a href='#' onclick='print(); return false;' accesskey='p'>Print</a> - ";
	if (sys.allowPaging) {
		if (sys.col0 - sys.cols >= 0 || sys.row0 - sys.rows >= 0) {
			out += "<a href='#' onclick='sys.row0=0; sys.col0=0; sys.currCol=0; sys.currRow=0; scroll(); display(); return false;'>Home</a> - ";
		}
		if (sys.col0 - sys.cols >= 0) {
			out += "<a href='#' onclick='sys.col0 -= sys.cols; sys.currCol -= sys.cols; display(); return false;'>&lt;&lt;</a> - ";
		} else
			out += "&lt;&lt; - ";
		out += "<a href='#' onclick='sys.col0 += sys.cols; sys.currCol += sys.cols; display(); return false;'>&gt;&gt;</a> - ";
		if (sys.row0 - sys.rows >= 0) {
			out += "<a href='#' onclick='sys.row0 -= sys.rows; sys.currRow -= sys.rows; display(); return false;'>Up</a> - ";
		} else
			out += "Up - ";
		out += "<a href='#' onclick='sys.row0 += sys.rows; sys.currRow += sys.rows; display(); return false;'>Down</a> -&nbsp;";
		out += "</td><td nowrap>";
	...
	out += "<input type='text' value='' title='Position' id='field' style='width:30px; text-align:center;' onfocus='sys.active=\"position\";' onblur='sys.active=\"content\";' accesskey='g'> -&nbsp;";
	out += "</td><td nowrap style='width:100%;'>";
	out += "<iframe src='about:blank' id='multiline'></iframe>";
	out += "<input type='text' value='' title='Formula' id='value' style='width:67%;' disabled onmouseover='previewValue();' onkeyup='previewValue();'> ";
	out += "<input type='text' title='Style' value='' id='styling' style='width:33%;' disabled onmouseover='previewValue();' onkeyup='previewValue();'> ";
	out += "</td><td nowrap>";
	if (sys.isWriteable) {
		out += "&nbsp; <input type='button' value='Save' id='save' onclick='saveCell();' disabled>&nbsp;";
		out += "<input type='button' value='X' id='cancel' onclick='cancelCell();' disabled> - ";
	} else
		out += "&nbsp; ";
	if (sys.view == "values") {
		out += "<a href='#' onclick='sys.view=\"formulas\"; display(); return false;'>Values</a> - ";
	} else if (sys.view == "formulas") {
		out += "<a href='#' onclick='sys.view=\"styles\"; display(); return false;'>Formulas</a> - ";
	} else {
		out += "<a href='#' onclick='sys.view=\"values\"; display(); return false;'>Styles</a> - ";
	}
	...
	if (sys.showColumnGroups) {
		...
		for (var i = sys.col0; i < sys.cols + sys.col0; i++) {
			...
			if (colGroupTitle && (!sys.marks[-2] || !sys.marks[-2][i])) {
				...
				if (colSpan) {
					if (!sys.marks[-2])
						sys.marks[-2] = new Array();
					for (var s = i + 1; s < i + colSpan; s++)
						sys.marks[-2][s] = new Array(-2, i);
				}
		...
		for (var col = sys.col0; col < sys.cols + sys.col0; col++) {
			if (sys.view == "values") {
				if (!sys.marks[row] || !sys.marks[row][col]) {
					...
					if (colSpan) {
						if (!sys.marks[row])
							sys.marks[row] = new Array();
						for (var s = col + 1; s < col + colSpan; s++)
							sys.marks[row][s] = new Array(row, col);
					}
					var rowSpan = parseInt(getCellStyle(row, col, "rowspan"));
					if (rowSpan) {
						for (var s = row + 1; s < row + rowSpan; s++) {
							if (!sys.marks[s])
								sys.marks[s] = new Array();
	...
	for (var i = sys.col0; i < sys.cols + sys.col0; i++)
		out += "<th class='empty'><img src='' style='width:" + sys.colMinWidth + "; height:0px;'></th>";
	out += "</tr>";
	out += "</table></div>";
	out += "<div class='footer' id='footer' onmouseover='getObj(\"status\").innerHTML=\"\";'>&nbsp;";
	if (sys.isWriteable) {
		out += "Insert: ";
		out += "<a href='#' onclick='insertRow(); return false;'>Row</a> - ";
		out += "<a href='#' onclick='insertColumn(); return false;'>Column</a> - ";
		out += "Delete: ";
		out += "<a href='#' onclick='if (confirm(\"Really delete entire row ?\")) deleteRow(); return false;'>Row</a> - ";
		out += "<a href='#' onclick='if (confirm(\"Really delete entire column ?\")) deleteColumn(); return false;'>Column</a> - ";
	}
	out += "Sort: ";
	out += "<a href='#' onclick='sort(1); return false;'>asc.</a> - ";
	out += "<a href='#' onclick='sort(0); return false;'>desc.</a> - ";

	if (sys.isWriteable) {
		out += "<a href='#' onclick='cutcopy(\"cut\",\"#FFDDDD\"); return false;' title='Alt-x' accesskey='x'>Cut</a> - ";
		out += "<a href='#' onclick='cutcopy(\"copy\",\"#DDDDFF\"); return false;' title='Alt-c' accesskey='c'>Copy</a> - ";
		out += "<a href='#' onclick='paste(); return false;' title='Alt-v' accesskey='v'>Paste</a> - ";
		out += "<a href='#' onclick='if (confirm(\"Really empty cell(s) ?\")) removeSelectedCell(); return false;' title='Alt-e' accesskey='e'>Empty</a> - ";
	}
	out += "Export: ";
	out += "<a href='#' onclick='save(\"js\"); return false;'>JS</a> - ";
	out += "<a href='#' onclick='save(\"csv\"); return false;'>CSV</a> - ";
	out += "<a href='#' onclick='save(\"tsv\"); return false;'>TSV</a>";
	out += "</div>";
	...
	if (sys.clipboard_mode != "") {
		var color = "#DDDDFF"
		if (sys.clipboard_mode == "cut")
			color = "#FFDDDD";
		if (sys.clipboard_range.length > 0) {
			highlightRange(sys.clipboard_range, "", color);
		} else {
			var obj = resolveCell(sys.clipboard_row, sys.clipboard_col);
			if (obj)
...
function previewField() {
	...
	if (val.indexOf("<img") != -1)
		obj2.style.height = obj2.offsetHeight + "px";
	if (sys.currRow == -1 || sys.currCol == -1)
		highlightCellHeader(sys.currRow, sys.currCol);
}
...
function isWritable(style) {
	if (style.indexOf("readonly:true") == -1 && sys.isWriteable)
		return true;
	return false;
}
...
function getCells(row, col, item) {
	if (sys.cells[row] && sys.cells[row][col] && sys.cells[row][col].length == 4)
		return sys.cells[row][col][item];
	return "";
}
...
function setCells(row, col, item, value) {
	if (!sys.cells[row])
		sys.cells[row] = new Array();
	if (!sys.cells[row][col])
		sys.cells[row][col] = new Array("", "", "", "");
...
function load(code) {
	...
	try {
		...
		alert("Error parsing data: " + err + " i=" + i + " cells=\"" + dbCells[i] + "\"");
		return;
	}
	if ( typeof sys.cells[-2] != "undefined")
		sys.showColumnGroups = true;
	else
		sys.showColumnGroups = false;
	sys.active = "content";
...
function loadTSV(code) {
	code = code.replace(/^\/\/.*?\n/g, "").replace(/\r/g, "").split("\n");
	for (var i = 0; i < code.length; i++) {
		code[i] = "[\"" + code[i].replace(/"/g, "\\\"").replace(/	/g, "\",\"") + "\"]";
		// tab
		if (i != code.length - 1)
			code[i] += ",";
	}
	...
	for (var i = -2; i < sys.cells.length; i++) {
		if (sys.cells[i]) {
			for (var i2 = -1; i2 < sys.cells[i].length; i2++) {
				if (!sys.cells[i][i2])
					continue;
				if (sys.cells[i][i2][0] == "" && sys.cells[i][i2][1] == "")
					continue;
				out += "  [" + i2 + "," + i + ",\"" + strescape(sys.cells[i][i2][0]) + "\",\"" + strescape(sys.cells[i][i2][1]) + "\"";
				if (sys.cells[i][i2][2] && isNaN(sys.cells[i][i2][2]))
					out += ",\"" + strescape(sys.cells[i][i2][2]) + "\"";
				else if (sys.cells[i][i2][2])
					out += "," + sys.cells[i][i2][2];
		...
		for (var i = 0; i < sys.cells.length; i++) {
			if (sys.cells[i]) {
				for (var i2 = col0; i2 < sys.cells[i].length; i2++) {
					if (!sys.cells[i][i2 + 1] && sys.cells[i][i2])
						sys.cells[i][i2] = "";
					if (!sys.cells[i][i2 + 1] && !sys.cells[i][i2])
						continue;
					// TODO2 change formulas ?
...
function scrollDown() {
	...
	if (obj && (posY + obj.offsetHeight + 100) > height) {
		...
		if (newHeight > getObj("content").scrollTop)
			getObj("content").scrollTop = newHeight;
	}
	...
	if (obj && (posX + obj.offsetWidth + 100) > width) {
		...
		if (newWidth > getObj("content").scrollLeft)
			getObj("content").scrollLeft = newWidth;
	}
}
...
function cutcopy(mode, color) {
	if (!sys.isWriteable)
		return;
	if (sys.clipboard_range.length > 0) {
		highlightRange(sys.clipboard_range, "", "");
	} else {
		if (sys.clipboard_mode) {
			var obj = resolveCell(sys.clipboard_row, sys.clipboard_col);
			if (obj)
	...
	sys.clipboard_mode = mode;
	sys.clipboard_row = sys.currRow;
	sys.clipboard_col = sys.currCol;
	sys.clipboard_range = getMultiRange(sys.multiRange);
	if (sys.multiRange.length > 0) {
...
function paste() {
	if (sys.clipboard_range.length > 0) {
		var sRange = getMultiRange(sys.clipboard_range);
		...
		if (sys.clipboard_mode == "cut") {
			for (var row = sRange[0]; row <= sRange[2]; row++) {
				for (var col = sRange[1]; col <= sRange[3]; col++) {
					if ((row < cRange[0] || row > cRange[2]) || (col < cRange[1] || col > cRange[3]))
						removeCell(row, col);
				}
			}
			sys.clipboard_mode = "";
		...
		if (sys.multiRange.length > 0 && (sys.multiRange[0] != sys.multiRange[2] || sys.multiRange[1] != sys.multiRange[3])) {
			...
			if (sys.clipboard_mode == "cut" && (sys.clipboard_row < cRange[0] || sys.clipboard_row > cRange[2] || sys.clipboard_col < cRange[1] || sys.clipboard_col > cRange[3])) {
				removeCell(sys.clipboard_row, sys.clipboard_col);
			}
		} else {
			copyCell(sys.clipboard_row, sys.clipboard_col, sys.currRow, sys.currCol);
			if (sys.clipboard_mode == "cut" && (sys.clipboard_row != sys.currRow || sys.clipboard_col != sys.currCol)) {
				removeCell(sys.clipboard_row, sys.clipboard_col);
			}
		}
		if (sys.clipboard_mode == "cut")
...
function sortNum(val) {
	if (isNaN(val))
		return val.charAt(0).toLowerCase().charCodeAt(0);
...
function sort(asc) {
	if (sys.multiRange.length > 0) {
		...
		for (var row = cRange[0]; row <= cRange[2]; row++) {
			for (var col = cRange[1]; col <= cRange[3]; col++) {
				if (!cCells[row])
					cCells[row] = new Array();
				if (sys.cells[row] && sys.cells[row][col])
					cCells[row][col] = sys.cells[row][col];
			}
		}
		cCells = cCells.sort(sortCells);
		if (!asc)
			cCells = cCells.reverse();
		for (var row = cRange[0]; row <= cRange[2]; row++) {
			for (var col = cRange[1]; col <= cRange[3]; col++) {
				if (!sys.cells[row])
					sys.cells[row] = new Array();
		...
		sys.cells = sys.cells.sort(sortCells);
		if (!asc)
			sys.cells = sys.cells.reverse();
	}
	display();
}
...
function mouseoverCell(row, col) {
	if (getObj("styling").disabled) {
		if (row == sys.currRow && col == sys.currCol && getObj("field").value) {
			editCell(row, col, 0);
		} else {
			markCell(row, col);
			buildStatus(row, col);
...
function highlightCellHeader(row, col) {
	...
	if (obj)
		obj.className = "border_highlight";
	if (row >= -1) {
...
function saveCell() {
	var changed = setCellsR(sys.currRow, sys.currCol, 0, getObj("value").value);
	var changed2 = setCellsR(sys.currRow, sys.currCol, 1, getObj("styling").value);
	if (changed || changed2) {
		if (!sys.autoRecalc) {
			disableEdit();
			previewValue();
...
function cancelCell() {
	disableEdit();
	getObj("value").value = getCellsR(sys.currRow, sys.currCol, 0);
	getObj("styling").value = getCellsR(sys.currRow, sys.currCol, 1);
	previewValue();
}
...
function resolveCell(row, col) {
	...
	if (!obj && sys.marks[row] && sys.marks[row][col]) {
		obj = getObj(sys.marks[row][col][0] + "_" + sys.marks[row][col][1]);
	}
	return obj;
}
...
function htmlEscape(str, fill) {
	...
	if (str.indexOf("html:") == 0) {
		...
		str = str.replace(/'/g, "&#039;").replace(/"/g, "&quot;");
		// quotes
		str = str.replace(/([^\s$]+@[^\s$]+)/g, "<a href='mailto:$1'>$1</a>");
		// make emails appear as a link
		str = str.replace(/img:((http[s]?:\/\/[^\s$]+\.(gif|jpg|png))|(graphs\.php\?.*?$))/g, "<img src='$1'>");
		// display images
		str = str.replace(/(^|[^'])(http[s]?:\/\/[^\s$]+)/g, "$1<a target='_blank' href='$2'>$2</a>");
		// make http(s)://... appear as link
		str = str.replace(/\\n/g, "<br>");
	}
	return str;
}
...
function check_js(code) {
	...
	pattern = /top|parent|window|document|this|eval|alert|settimeout|xmlhttp|ajax|load|check|sys\./i;
	if (pattern.test(code_a))
		return "";
	return code;
}
...
function formatStyle(style, value) {
	if (style.indexOf("text-align:") == -1 && value.length > 0 && !isNaN((value + "").replace(/[$%,]/g, "").replace("&euro;", ""))) {
		...
		if (value < 0 && style.indexOf("color:") == -1)
			style += "color:#FF0000;";
	}
...
function formatValue(value, style) {
	if (style.indexOf("format:") != -1) {
		if (style.indexOf("format:euro") != -1)
			value = formatNumber(value) + " &euro;";
		else if (style.indexOf("format:dollar") != -1)
			value = "$" + formatNumber(value);
		else if (style.indexOf("format:percent") != -1)
			value = (value * 100).toFixed(2) + "%";
		else if (style.indexOf("format:number") != -1)
			value = formatNumber(value);
		else if (style.indexOf("format:datefulltime") != -1)
			value = formatDateFullTime(value);
		else if (style.indexOf("format:datetime") != -1)
			value = formatDateTime(value);
		else if (style.indexOf("format:datefull") != -1)
			value = formatDateFull(value);
		else if (style.indexOf("format:date") != -1)
			value = formatDate(value);
		else if (style.indexOf("format:time") != -1)
...
function avg(arr) {
	...
	for (var i = 0; i < arr.length; i++)
		result += arr[i];
	return Math.round(1000 * (result / arr.length)) / 1000;
	// round to 0.000
}
